...
def test_create_subject(token, test_subject):
    request_data = {"name": "Physics"}
    response = client.post(
        "/admin/subjects", json=request_data, headers=get_authorization_header(token)
    )
...
def test_edit_subject(token, test_subject):
    request_data = {"name": "Physics"}
    response = client.put(
        "/admin/subjects/1", json=request_data, headers=get_authorization_header(token)
    )
    db = TestingSessionLocal()
...
def test_delete_subject(token, test_subject):
    response = client.delete(
        "/admin/subjects/1", headers=get_authorization_header(token)
    )
    db = TestingSessionLocal()
    model = db.query(Subject).filter(Subject.id == 1).first()
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_delete_subject_not_exist_subject(token):
    response = client.delete(
        "/admin/subjects/999", headers=get_authorization_header(token)
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {"detail": "Subject with id: 999 does not exist"}
...
def test_delete_subject_related_grades_error(token, test_grade):
    ...
    assert response.json() == {
        "detail": "Subject with id: 1 cannot be deleted because it is associated with table: "
        "grades."
    }
