...
def test_create_class(token, test_class):
    request_data = {"name": "1B"}
    response = client.post(
        "/admin/classes", json=request_data, headers=get_authorization_header(token)
    )
...
def test_edit_class(token, test_class):
    request_data = {"name": "1B"}
    response = client.put(
        "/admin/classes/1", json=request_data, headers=get_authorization_header(token)
    )
    db = TestingSessionLocal()
...
def test_edit_class_not_exist_class(token):
    request_data = {"name": "1B"}
    response = client.put(
        "/admin/classes/999", json=request_data, headers=get_authorization_header(token)
    )
...
def test_delete_class(token, test_class):
    response = client.delete(
        "/admin/classes/1", headers=get_authorization_header(token)
    )
    assert response.status_code == status.HTTP_204_NO_CONTENT
    db = TestingSessionLocal()
    model = db.query(Class).filter(Class.id == 1).first()
...
def test_delete_class_not_exist_class(token):
    response = client.delete(
        "admin/classes/999", headers=get_authorization_header(token)
    )
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {"detail": "Class with id: 999 does not exist"}
