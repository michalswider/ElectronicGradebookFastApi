...
def test_get_attendance_for_student_in_subject_attendance_not_found(test_student, test_subject):
    response = client.get('teacher/attendance/subject/1/student/1')
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_edit_attendance_status_user_id_not_found(test_attendance):
    request_data = {'status': 'absent'}
...
def test_delete_attendance_data_not_found(test_attendance):
    response = client.delete('teacher/delete-attendance/1/1/999')
    assert response.status_code == status.HTTP_404_NOT_FOUND
