...
def test_add_attendance(token, test_attendance):
    ...
    response = client.post('/teacher/add-attendance', json=request_data, headers=get_authorization_header(token))
...
def test_show_student_attendance(token, test_attendance):
    response = client.get('/teacher/show-student-attendance/1', headers=get_authorization_header(token))
...
def test_show_student_attendance_user_not_found(token):
    response = client.get('/teacher/show-student-attendance/999', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': 'User with id: 999 not found'}
...
def test_get_attendance_for_class_on_date(token, test_attendance):
    response = client.get('teacher/attendance/class/1/date?date=2024-09-04', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_200_OK
...
def test_get_attendance_for_class_on_date_class_not_exist(token):
    response = client.get('teacher/attendance/class/999/date?date=2024-09-04', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {'detail': 'Class with id: 999 does not exist'}


def test_get_attendance_for_class_on_date_attendance_not_found(token, test_student_with_class):
    response = client.get('teacher/attendance/class/1/date?date=2024-09-04', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_get_attendance_for_student_in_subject(token, test_attendance):
    response = client.get('teacher/attendance/subject/1/student/1', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_200_OK
...
def test_get_attendance_for_student_in_subject_subject_not_exist(token):
    response = client.get('teacher/attendance/subject/999/student/1', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {'detail': 'Subject with id: 999 does not exist'}


def test_get_attendance_for_student_in_subject_user_id_not_found(token, test_subject):
    response = client.get('teacher/attendance/subject/1/student/999', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': 'User with id: 999 not found'}


def test_get_attendance_for_student_in_subject_attendance_not_found(token, test_student, test_subject):
    response = client.get('teacher/attendance/subject/1/student/1', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_edit_attendance_status(token, test_attendance):
    ...
    response = client.put('teacher/edit-attendance/1/1/1', json=request_data, headers=get_authorization_header(token))
    db = TestingSessionLocal()
...
def test_edit_attendance_status_subject_not_exist(token, test_attendance):
    request_data = {'status': 'absent'}
    response = client.put('teacher/edit-attendance/1/999/1', json=request_data, headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {'detail': 'Subject with id: 999 does not exist'}
...
def test_edit_attendance_status_user_id_not_found(token, test_attendance):
    request_data = {'status': 'absent'}
    response = client.put('teacher/edit-attendance/999/1/1', json=request_data, headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': 'User with id: 999 not found'}
...
def test_edit_attendance_status_attendance_data_not_found(token, test_attendance):
    request_data = {'status': 'absent'}
    response = client.put('teacher/edit-attendance/1/2/1', json=request_data, headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
def test_delete_attendance(token, test_attendance):
    response = client.delete('teacher/delete-attendance/1/1/1', headers=get_authorization_header(token))
    db = TestingSessionLocal()
    model = db.query(Attendance).filter(Attendance.id == 1).first()
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_delete_attendance_user_id_not_found(token, test_attendance):
    response = client.delete('teacher/delete-attendance/999/1/1', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': 'User with id: 999 not found'}


def test_delete_attendance_subject_not_exist(token, test_attendance):
    response = client.delete('teacher/delete-attendance/1/999/1', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json() == {'detail': 'Subject with id: 999 does not exist'}


def test_delete_attendance_data_not_found(token, test_attendance):
    response = client.delete('teacher/delete-attendance/1/1/999', headers=get_authorization_header(token))
    assert response.status_code == status.HTTP_404_NOT_FOUND
