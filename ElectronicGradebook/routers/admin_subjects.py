...
async def create_subject(request: Request, create_subject_request: CreateSubjectRequest, db: db_dependency):
...
async def edit_subject(request: Request, db: db_dependency, create_subject_request: CreateSubjectRequest,
...
async def delete_subject(request: Request, db: db_dependency, subject_id: int = Path(gt=0)):
    user = request.state.user
    verify_admin_user(user)
    validate_subject_exist(user, subject_id, db)
