...
from ..response_models.attendance import (
    map_attendance_for_class_on_date_to_response,
    map_attendance_for_student_in_subject, map_student_attendance_to_response)
from ..routers.auth import get_db
from ..schemas.attendance import (AddAttendanceRequest,
                                  EditAttendanceStatusRequest)
from ..services.attendance_service import (create_attendance,
                                           delete_attendances, edit_attendance)
from ..services.validation_service import (
    validate_attendance_data, validate_attendance_for_class_on_date,
    validate_attendance_for_student_in_subject, validate_attendance_status,
    validate_class_exist, validate_student_attendance, validate_subject_exist,
...
async def add_attendance(
    request: Request, db: db_dependency, add_attendance_request: AddAttendanceRequest
):
...
async def show_student_attendance(
...
async def get_attendance_for_class_on_date(
    request: Request,
    db: db_dependency,
...
@router.get(
    "/attendance/subject/{subject_id}/student/{student_id}",
    status_code=status.HTTP_200_OK,
)
async def get_attendance_for_student_in_subject(
    request: Request,
    db: db_dependency,
    ...
    validate_user_id(student_id, db, user, role="student")
    attendance_model = validate_attendance_for_student_in_subject(
        student_id, subject_id, user, db
    )
    return map_attendance_for_student_in_subject(attendance_model)


@router.put(
    "/edit-attendance/{student_id}/{subject_id}/{attendance_id}",
    status_code=status.HTTP_204_NO_CONTENT,
)
async def edit_attendance_status(
    request: Request,
    db: db_dependency,
    ...
    validate_subject_exist(user, subject_id, db)
    validate_user_id(student_id, db, user, role="student")
    attendance_model = validate_attendance_data(
        attendance_id, student_id, subject_id, user, db
    )
...
@router.delete(
    "/delete-attendance/{student_id}/{subject_id}/{attendance_id}",
    status_code=status.HTTP_204_NO_CONTENT,
)
